...
def load_data():
    ...
    # Normalize types for Streamlit widgets
    ...
    if "Archived" in df.columns:
        df["Archived"] = (
            df["Archived"].astype(str).str.strip().str.lower().isin(["true", "1", "yes", "y"])
        )
    else:
        df["Archived"] = False
...
def filters_ui(df: pd.DataFrame):
    with st.expander("🔎 Filters", expanded=True):
        ...
        with c2:
            store_incorrect = st.text_input("In-Correct contains")
        with c3:
            store_fitment = st.text_input("Fitment Completed contains")
        with c4:
            date_range = st.date_input("eComm Request date range", value=())
        c5, c6 = st.columns(2)
        with c5:
    ...
    if isinstance(date_range, (list, tuple)) and len(date_range) == 2:
        ...
        if start and end:
            ...
            mask &= req_dates.between(pd.Timestamp(start), pd.Timestamp(end), inclusive="both")
    if q:
        ql = q.lower()
...
def new_entry_form(user_email: str):
    with st.form("new_entry"):
        ...
        c1, c2, c3, c4 = st.columns(4)
        with c1:
            req_date = st.date_input("Date of eComm Request", value=date.today())
        with c2:
            order_num = st.text_input("Order Number", max_chars=50)
        with c3:
            status = st.selectbox("Status", STATUS_OPTIONS, index=0)
        with c4:
            requested_by = st.selectbox("Requested By", REQUESTED_BY_OPTIONS, index=0)

        c5, c6, c7, c8 = st.columns(4)
        with c5:
            incorrect_store = st.text_input("In-Correct")
        with c6:
            fitment_store = st.text_input("Store - Fitment Completed")
        with c7:
            amount = st.text_input("Amount", placeholder="$0.00")
        with c8:
            amount_type = st.selectbox("Amount Type", [""] + AMOUNT_TYPE_OPTIONS, index=0)

        c9, c10 = st.columns(2)
        with c9:
            finance_date = st.date_input("Date Finance Updated", value=None, format="YYYY-MM-DD")
        with c10:
        ...
        with t1:
            template = st.selectbox("Template", ["Standard","Scenario 2","Scenario 3","Scenario 4"])
        with t2:
...
def edit_selected_rows(df: pd.DataFrame, user_email: str):
    ...
    ed = st.data_editor(
        ...
        column_config={
            ...
            "Date of eComm Request": st.column_config.DateColumn(format="YYYY-MM-DD"),
            "Date Finance Updated": st.column_config.DateColumn(format="YYYY-MM-DD"),
            "Amount": st.column_config.TextColumn(),
            "Amount Type": st.column_config.SelectboxColumn(options=AMOUNT_TYPE_OPTIONS),
            "Requested By": st.column_config.SelectboxColumn(options=REQUESTED_BY_OPTIONS),
            "Reason": st.column_config.TextColumn(),
            "Email Subject": st.column_config.TextColumn(),
            "Email Body": st.column_config.TextColumn(),
            "Email Sent At": st.column_config.TextColumn(),
...
def kpi_bar(df: pd.DataFrame):
    """KPI tiles at top of Tracker. Vectorized & NaT-safe."""
    ...
    completed_mask = (df.get("Status") == "Completed") & fin_dates.between(start_of_week, today, inclusive="both")
    new_mask       = req_dates.between(start_of_week, today, inclusive="both")
    open_mask      = df.get("Status").isin(["Flagged", "In-Progress"]) & (~df.get("Archived").fillna(False))
    archived_mask  = df.get("Archived").fillna(False)
...
def main():
    ...
    tabs = st.tabs(["📋 Tracker","➕ New Entry","📧 Email Tools","📊 Analytics","⚙️ Admin / Export","❓ Help"])

    # Tracker
    with tabs[0]:
        df = load_data()
        filtered = filters_ui(df)      # filters first
        kpi_bar(filtered)              # KPIs on filtered/cleaned data

        st.subheader("📦 Current Requests")
        st.caption("Use filters above to narrow results. Click a cell to edit, then 'Save changes'.")
        st.dataframe(filtered.drop(columns=["Email Body"], errors="ignore"), use_container_width=True, hide_index=True)

        st.divider()
        st.subheader("Inline Edit Mode")
        st.caption("Edit directly below, then save.")
        updated = edit_selected_rows(filtered, user_email)
        if len(updated) != len(df):
            # Merge edits back into original by index
            df.loc[updated.index, :] = updated
            save_tracker(df)
